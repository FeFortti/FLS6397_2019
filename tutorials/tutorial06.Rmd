...
Use o comando View para visualizar o resultado da coluna criada à direita do banco de dados. Simples, não? Basta inserimos dentro do comando _mutate_ a expressão da transformação que queremos.
...
Exceto o uso da palavra _filter_, não há nada de novo para nós. Nós já vimos condições tipo uf == "ES", que indica que apenas as linhas cuja variável _uf_ assumo valor igual a ES devem ser consideradas. Além da igualdade, poderíamos usar outros símbolos: maior (>). maior ou igual (>=), menor (<), menor ou igual (<=) e diferente (!=). 
