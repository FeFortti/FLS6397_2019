...
A seguir, criamos um objeto "tabela", que mantém a conexão direto com a tabela "pagamentos201701" que está no banco de dados "PBF".
...
```{r}
conexao <- src_mysql(dbname = "fake_data", 
                     host = "db4free.net", 
                     port = 3306, 
                     user = "jonnyphillips",
                     password = "usp_test")
...
Uma vez conectados com o servidor, fazemos a conexão direta com a tabela "TABLE 1", que é a que nos interessa (pode haver inúmeras tabelas em uma base de dados). Vamos chamar o objeto criado de "fake". Será este objeto que manipularemos com a gramática que aprendemos anteriormente. 

```{r}
fake <- tbl(conexao, "TABLE 1")
```
...
```{r}
tweats2 <- tweats %>% rename(tweat_id = id)
tabela_join <- tweats2 %>% left_join(comments, "tweat_id")
head(tabela_join)
```
...
tweats3 <- tweats %>% 
  ...
  filter(str_detect(message,"!"))  # filtra por comentarios que terminam em !
