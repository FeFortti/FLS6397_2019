...
Gráficos de linha exigem, em geral, um pouco mais de preparação de nossos dados. A variável x pode ser discreta ou contínua, mas precisa ser _ordenado_ para que as linhas façam sentido. Só precisamos mexer o data.frame fora do _ggplot2_ e colocá-lo antes do pipe. Por exemplo, podemos visualizar apenas os dados para as mulheres.
...
Note uma coisa chata: existe dois símbolos conetando as nossas ações agora: um pipe ( _%>%_ ) e um "+". Usamos o pipe com data.frames, e o "+" com camadas de gráficos depois da linha _ggplot()_. 
...
Vamos modificar os cores de nosso gráfico de linha. O nosso variável distinguida pela cor é o sexo, que é discreto, e nós queremos colorir as linhas, não as áreas, então precisamos usar _scale\_color\_brewer_. Eu gosto de uma escala/palette se chame 'Accent' então adicionamos uma nova linha no código.
...
Com o banco de dados _flights_, filtrar os dados para voos partindo do aeroporto LGA no dia 20 de maio ('day' e 'month'). Depois, crie um gráfico de dispersão/texto que compara a hora de partida com distancia, que coloca os nomes de destinos no gráfico, e, usando um 'scale', para que o cor de cada texto reflete o atraso de partido desse voo.
...
Os resultados de uma simples regressão pode ser complicada e confusa. Exibindo gráficos dos resultados podem ajudar a comunicar nossas análises ao nosso público. Por exemplo, imagine que queremos mostrar os resultados da regressão de renda sobre idade e número de filhos.

```{r}
fake %>% lm(renda ~ idade + filhos, data=.) 
```
...
Depois, podemos tirar o intercept que normalmente não nos interessa muito usando _filter_, e calcule os intervalos de confiança de 95\% usando _mutate_.
...
fake %>% lm(renda ~ idade + filhos, data=.) %>% 
  ...
  geom_segment(aes(x=term,xend=term,y=Conf.lo,yend=Conf.hi))
```
