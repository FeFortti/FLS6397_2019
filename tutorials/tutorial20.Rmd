...
_rvest_ é um pacote que faz parte de um "universo" de pacotes, chamado "tidyverse". Se você "chamar" o pacote _tidyverse_, não precisará chamar _rvest_, pois a função do _tidyverse_ é carregar os pacotes que o compõem.
...
Há algumas informações sobre as proposições: autor, documento e uma breve descrição.
...
```{r}
url_base <- "https://www.al.sp.gov.br/alesp/pesquisa-proposicoes/?direction=acima&lastPage=45&currentPage=1&act=detalhe&idDocumento=&rowsPerPage=20&currentPageDetalhe=1&tpDocumento=&method=search&text=merenda&natureId=&legislativeNumber=&legislativeYear=&natureIdMainDoc=&anoDeExercicio=&strInitialDate=&strFinalDate=&author=&supporter=&politicalPartyId=&stageId="
```
...
Também podemos observar que a pesquisa retornou 45 páginas ("lastPage=45") e que a página atual (2) é a de número 1 ("currentPage=1") -- a página 1 da pesquisa é numerada como 0 nesta ferramenta de busca, mas o padrão muda de página para página.

Podemos ver que há muitas páginas de resultado para a palavra-chave que utilizamos. Nosso desafio é conseguir "passar" de forma eficiente por elas, ou seja, acessar os 45 links e "raspar" o seu conteúdo. Para isso, usaremos uma função essencial na programação, o "for loop". Vamos rever um pouco loops:
...
## Substituição com str_replace

Ótimo! Já temos alguma intuição sobre como loops funcionam. Podemos agora fazer "passar" pelas páginas que contêm a informação que nos interessa. Temos que escrever uma pequena instrução que indique ao programa que queremos passar pelas páginas de 1 a 45, substituindo apenas o número da página atual -- "currentPage" -- no endereço URL que guardamos no objeto url_base.

Nos falta, porém, uma função que nos permita substituir no texto básico do URL ("url_base") os números das páginas.

Há mais de uma opção para realizar essa tarefa, mas aqui usaremos a função _str\_replace_, do pacote _stringr_, que vimos na aula de mineração de texto. Você pode usar também a função _gsub_ do "base", se preferir. Carregue o pacote _stringr_:

```{r}
library(stringr)
```

Com a função _str\_replace_ podemos substituir um pedaço de um objeto de texto por outro, de acordo com o critério especificado.
...
texto_final <- str_replace(meu_texto, o_que_procuro_para_susbtituir, o_que_quero_substituir_por)
...
Vamos substituir na URL da página 2 da nossa busca o número por algo que "guarde o lugar" do número de página. Esse algo é um "place holder" e pode ser qualquer texto. No caso, usaremos "NUMPAG". Veja abaixo onde "NUMPAG" foi introduzido no endereço URL. 
...
```{r}
url_base <- "https://www.al.sp.gov.br/alesp/pesquisa-proposicoes/?direction=acima&lastPage=45&currentPage=NUMPAG&act=detalhe&idDocumento=&rowsPerPage=20&currentPageDetalhe=1&tpDocumento=&method=search&text=merenda&natureId=&legislativeNumber=&legislativeYear=&natureIdMainDoc=&anoDeExercicio=&strInitialDate=&strFinalDate=&author=&supporter=&politicalPartyId=&stageId="
```

Por exemplo, se quisermos gerar o link da página 6, podemos escrever:

```{r}
url_p7 <- str_replace(url_base, "NUMPAG", "6")

print(url_p7)
```

Note que estamos substituindo um texto por outro. A função _str\_replace_ não substitui um texto por número.
...
```{r}
i = "6"
url_p7 <- str_replace(url_base, "NUMPAG", i)
...
for(i in as.character(0:5)){
  url_pagina <- str_replace(url_base, "NUMPAG", i)
  print(url_pagina)
}
```

Lembre-se transformar o vetor "0:5" em texto para poder fazer a substituição.
...
```{r}
url_cryptorave <- "https://pt.wikipedia.org/wiki/CryptoRave"
pagina <- read_html(url_cryptorave)
print(pagina)
```
...
Um detalhe fundamental do resultado da função _html\_table_ é que o resultado dela é uma lista. Por que uma lista? Porque pode haver mais de uma tabela na página e cada tabela ocupará uma posição na lista. Para R, uma lista pode combinar objetos de diversas classes: vetores, data frames, matrizes, etc.
...
Podemos juntar tudo que vimos até agora: loop com a função "for", substituição com "str\_replace", captura de tabelas em HTML, listas e seus elementos.
...
São 50 observações (5 páginas com 10 resultados) e 2 variáveis (autor e documento + descrição, que são, para a tabela, um campo único), exatamente como esperávamos. As 2 variáveis são do tipo "character" contêm as informações corretas. As 6 observações apresentam o resultado adequado, o que nos dá uma boa dica que que tudo ocorreu bem até a última página capturada.
