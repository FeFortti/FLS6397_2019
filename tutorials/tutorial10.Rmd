...
Dados espaciais são dados organizados por localização e permitem novos tipos de análise e visualizações. Explorar mapas em R nos permite praticar e estender muitas das ferramentas que aprendemos nas últimas semanas - análises de dados com dplyr, joins e gráficos com ggplot2.

# Mapeando Pontos Geográficos

Vamos começar a trabalhar com mapas a partir de um exemplo que, veremos, utilizará o cadastro de escolas que a Prefeitura Municipal de São Paulo disponibiliza [aqui](http://dados.prefeitura.sp.gov.br/).
...
Não há nada de extraordinário no arquivo. Há, porém, uma dupla de variáveis que nos permite trabalhar "geograficamente" com os dados: LATITUDE e LONGITUDE. "Lat e Long" são as informações fundamentais de um dos sistemas de coordenadas (coordinate reference system, CRS) mais utilizados para localização de objetos na superfície da terra.
...
Para analisar estes dados como dados espaciais precisamos informar quais são as variáveis de localização e transformá-lo em um objeto 'simple features' usando o biblioteca _sf_ e a função _st\_as\_sf_ para criar um objeto tipo "simple features". Lembre-se de instalar o pacote antes de carregá-lo.
...
O parâmetro 'coords' indica os colunas de latitude e longitude, e o 'crs' indica o sistema de coordenadas (a "projeção") que queremos usar. Na verdade, o sistema de coordenadas não é uma opção nossa - precisamos especificar o mesmo sistema de coordenadas com o qual os dados foram salvos. Às vezes isso é difícil de saber. Aqui, como parece que a longitude e latitude estão em graus (entre -180 e 180) é provável que devemos usar o sistema "WGS84" (um sistema de coordenadas geográficas (não projetadas)). Um 'shortcut' para especificar o WGS84 é usar o numero _4326_ no argumento 'crs'. Para outros shortcuts para sistemas de coordenadas, que podemos usar com dados salvos em outros sistemas de coordenadas, veja [http://epsg.io/](http://epsg.io/). Não se preocupe muito com sistemas de coordenadas - vamos discutí-los mais em aula.
...
4. No próprio data.frame temos duas colunas de NOMESC e SITUACAO, e uma terceira de 'geometry' - isso é uma coluna especial que contém as informações de localização para cada unidade. Aqui estão as coordenadas dos nossos pontos.
...
Pronto! Você já criou o seu primeiro mapa! O eixo _x_ é o longitude e o eixo _y_ é o latitude. Abaixo, vamos ver como a representar outros aspectos de nosso data.frame em mapas. Agora, temos uma outra abordagem para criar um mapa interativo, com um mapa de terra no fundo. Usamos o pacote 'mapview' e, de novo, não precisamos especificar nenhum parâmetro.
...
Como podemos visualizar este mapa? Exatamente como antes:
...
Quando trabalhamos com dados espaciais de diversas fontes (ex. as escolas e os municipios), __é essencial ter certeza que estamos trabalhando com a mesma projeção para todos os objetos__ - caso contrário nossos mapas podem não aparecer corretamente e nossas medidas espaciais serão imprecisas. Dê uma olhada na CRS/projeção de 'municipios' - está escrito "+proj=longlat +ellps=GRS80 +no_defs" - em contraste de CRS/projeção de 'escolas' - "+proj=longlat +datum=WGS84 +no_defs". O parte 'WGS84' é o mais comum então vamos transformar os dados dos municipios para a mesma projeção usando a função _st\_transform_ e o shortcut 4326 para WGS84. Agora temos tudo pronto para fazer análises ou mapas incorporando as duas camadas espaciais.
...
Se você abrir o objeto _municipios_ vai ver os detalhes de CRS/projeção e o data.frame dele mesmo, incluindo a coluna de 'geometry' e também todas as colunas normais, incluindo o código municipal (CD_GEOCODM). Isso é uma oportunidade para nós - se tivermos dados em todos os municípios, podemos simplesmente 'join' estes dados no shapefile e, em seguida, podemos visualizar mapas dessas variáveis.
...
Para visualizar a coluna 'pct_voto' num mapa, podemos trabalhar em ggplot normalmente. Para polígonos, colocamos o nome de coluna para o parâmetro _fill_ ( _color_ com pontos e linhas). Um mapa de todo o Brasil pode ser muito pesado, então vamos nos concentrar em São Paulo com um _filter_.
...
Além disso, podemos alterar as escalas de cores como em ggplot. 
...
Podemos colocar os pontos no mesmo mapa? Sim - apenas especificamos o parâmetro de dados para uma nova camada dentro de _geom\_sf_. (Isto assume que nós confirmamos que ambas as camadas têm a mesma projeção, como vimos acima).
...
Este mapa parece bem chato porque os dados são altamente assimétricos, com grandes outliers apenas na capital. Frequentemente com rasters é útil transformá-los em uma escala de log antes de visualizar. Vamos também limpar o fundo e adicionar uma escala de cores.
