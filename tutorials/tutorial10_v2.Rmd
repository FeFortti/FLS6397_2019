...
Antes de iniciar o tutorial, certifique-se de que os pacotes _sf_, _mapview_, _tmap_ e _cepespR_ são instalados.
...
```{r}
download.file("https://github.com/JonnyPhillips/FLS6397_2019/data/raw/master/Dilma_2010_segundo_turno.csv",destfile="Dilma_2010_segundo_turno.csv")

Dilma_2010 <- read_csv("Dilma_2010_segundo_turno.csv")

library(cepespR)
dados_eletorais <- get_elections(year=2010, 
                                 position="President",
...
dados_eletorais <- dados_eletorais %>% filter(NUM_TURNO==2) %>% 
  ...
  select(COD_MUN_IBGE, NOME_CANDIDATO, QTDE_VOTOS)

dados_eletorais %>% write_csv("Dilma_2010_segundo_turno.csv") 
```
