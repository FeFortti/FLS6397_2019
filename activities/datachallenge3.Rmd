...
Siga as instruções abaixo. Faça uma síntese de sua atividade e apresente apenas o essencial do código (você deve julgar o que é essencial).
...
Em todos os gráficos, insira título e rótulos dos eixos. Quando necessário, inclua legenda. Altere os elementos da geometria (cor, tamanho, formato, etc de linhas, pontos e barras) para tornar as visualizações o mais elegange possível. Explique no script brevemente - em duas ou três frases - o que podemos aprender a partir de cada gráfico.

Finalmente, seu arquivo de R deve calcular pelo menos 3 informações/estatísticas provenientes dos dados (exemplo: contagem de casos, médias de variáveis, somatórios).
